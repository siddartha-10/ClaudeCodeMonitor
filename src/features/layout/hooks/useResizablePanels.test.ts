...
describe("useResizablePanels", () => {
  ...
  it("reads stored sizes and clamps to bounds", () => {
    window.localStorage.setItem("claude-code-monitor.sidebarWidth", "999");
    window.localStorage.setItem("claude-code-monitor.rightPanelWidth", "100");
    window.localStorage.setItem("claude-code-monitor.planPanelHeight", "not-a-number");
  ...
  it("persists sidebar width changes and clamps max", () => {
    ...
    expect(hook.result.sidebarWidth).toBe(420);
    expect(window.localStorage.getItem("claude-code-monitor.sidebarWidth")).toBe(
      "420",
    );

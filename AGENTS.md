# ClaudeCodeMonitor Agent Guide

## Project Summary
ClaudeCodeMonitor is a macOS Tauri app that orchestrates Claude Code agents across local workspaces. The frontend is React + Vite; the backend is a Tauri Rust process that spawns `claude app-server` per workspace and streams JSON-RPC events.

## Key Paths

- `src/App.tsx`: composition root
- `src/features/`: feature-sliced UI, hooks, and local helpers
- `src/features/settings/components/SettingsView.tsx`: settings UI (projects, display, Claude)
...
- `src-tauri/src/claude_config.rs`: read/write Claude `config.toml` feature flags
...
- Backend spawns `claude app-server` using the `claude` binary.
...
- Experimental feature toggles: UI state in `src/features/settings/components/SettingsView.tsx`, shared types in `src/types.ts`, and sync to Claude `config.toml` via `src-tauri/src/claude_config.rs` + `src-tauri/src/settings.rs`.
...
- App settings live in `settings.json` under the app data directory (Claude path, default access mode, UI scale).
